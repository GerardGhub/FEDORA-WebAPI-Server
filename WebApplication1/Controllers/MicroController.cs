...
namespace WebApplication1.Controllers
{
    public class MicroController : ApiController
    {
        ...
        public HttpResponseMessage Get()
        {
			...
			--CAST(emp.last_used as date) as LAST_USED,
			--emp.last_used as LAST_USED,
			CONVERT(date, emp.last_used, 101) as LAST_USED,
			DATEDIFF(DAY,CONVERT(date, emp.last_used, 101),CAST(GETDATE() as date)) as MOVEMENT,
			emp.report,
			(ISNULL(t5.RECEIVING,0) + ISNULL(t6.ISSUE,0) - ISNULL(t2.MACRESERVED,0) - ISNULL(t7.OUTING,0) - 	CAST(emp.buffer_of_stocks as float)) / 100 as DIF,
		...
		FROM rdf_raw_materials emp
		         ...
		         LEFT JOIN ( select BC.theo_item_code, sum( BC.actual_qty ) as SCADA
                        from theo_logs_tbl BC where CAST(date_time as date) BETWEEN '2021-01-12' and GETDATE()
                        group by BC.theo_item_code ) t3
            on emp.item_code = t3.theo_item_code


					         LEFT JOIN ( select BC.item_code, sum(CAST(BC.formulation_qty as float))  as MACRESERVED
            ...
            on emp.item_code = t4.rp_item_code
                        ...
                        from rdf_microreceiving_entry BC where is_active='1' and BC.transaction_type='PO'
                        group by BC.r_item_code) t5
            on emp.item_code = t5.r_item_code


										         LEFT JOIN ( select BC.r_item_code, sum(CAST(REPLACE(BC.r_quantity,',','') as float))  as ISSUE
                        from rdf_microreceiving_entry BC where BC.transaction_type='Miscellaneous Receipt'
		...
		WHERE emp.is_active = 1 AND emp.Category='MICRO' ORDER BY emp.item_group ASC
